...
/**
 * @brief Handler for MSG_TYPE_I2C_MESSAGE messages
 *
 ...
 *
 * @param request Pointer to the host request message, use request->i2c_message for structured access
...
static uint8_t i2c_message_handler(const union request *request, struct response *response)
{
	...
	if (num_write_bytes > num_write_bytes  || num_read_bytes > remaining_read_size) {
